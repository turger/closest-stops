...
const formatStopTimes = (stopTimes) => {
  return _.uniqWith(stopTimes, _.isEqual).reduce((accumulator, stopTime) => {
...
const formatNextFiveStopTimes = (stopTimes) => {
  return Array.from(
    _.uniqWith(stopTimes, _.isEqual)
...
export const filterStops = (stops, filterFavorites, favoriteRoutes, hiddenVehicles) => {
  return Object.keys(stops).reduce((accumulator, stop) => {
    const filteredStopTimes = filterStopTimes(
      stops[stop].stopTimes,
      filterFavorites,
      favoriteRoutes,
      hiddenVehicles
    )
    if (filteredStopTimes.length !== 0)
      accumulator[stop] = { ...stops[stop], stopTimes: filteredStopTimes }
    return accumulator
  }, {})
}
...
export const filterStopsForMap = (stops) => {
  const stopsasd = Object.keys(stops).reduce(
    (accumulator, id) => ({
      ...accumulator,
      [id]: {
        id: id,
